...
from surface_Currents_M_functions1004 import get_fvcom,draw_basemap
...
for j in range(track_hours):#len(toltime)
    cpoints,psqus = get_obj.current_track(j,centerpoint,bordersidele)
...
crang = np.linspace(0,1.0, num=10) #color range
...
def animate(n): #del ax.collections[:]; del ax.lines[:]; ax.cla(); ax.lines.remove(line)        
    ...
    plt.suptitle('Current model ' + toltime[n].strftime("%d-%b-%Y %H:%M"))
    del ax.lines[:]   
